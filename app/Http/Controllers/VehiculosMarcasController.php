...
class VehiculosMarcasController extends Controller
{
    /**
    ...
    {   
        ...
        if($request->has('q')){
            if($request->get('q')!=""){
                ...
                $q=$search;
                $marcas=VehiculoMarca::where('nombre','LIKE', '%'.$search.'%');
                $marcas=$marcas->paginate(Config::get('global_settings.paginate'));                           
            }
        }

        return view('vehiculos.marca')->with(['marcas'=>$marcas,'q'=>$q]);
    }
    public function new()
    { 
